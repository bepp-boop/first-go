...
func TestADES100CompositeFix(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES100Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	...
	// Create the fix (this does NOT apply it yet)
	fix := autofix.ADES100FixComposite(
		"${{ inputs.name }}",
		0,
		`echo "Hello ${{ inputs.name }}"`,
	)
...
func TestADES101CompositeFix(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES101Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
...
func TestADES102CompositeFix(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES102Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	// Create the fix (this does NOT apply it yet)
	fix := autofix.ADES102FixComposite(
		"${{ github.event.issue.title }}",
		0,
		"Closing ${{ github.event.issue.title }}",
	)
...
func TestADES103CompositeFix(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES103Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	// Create the fix (this does NOT apply it yet)
	fix := autofix.ADES103FixComposite(
		"${{ github.event.issue.title }}",
		0,
		"Closing ${{ github.event.issue.title }}",
	)
...
func TestADES104CompositeFix(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES104Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	// Create the fix (this does NOT apply it yet)
	fix := autofix.ADES104FixComposite(
		"${{ github.event.inputs.file }}",
		0,
		"jq .version ${{ github.event.inputs.file }} -r",
	)
...
func TestADES105CompositeFix(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES105Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	// Create the fix (this does NOT apply it yet)
	fix := autofix.ADES105FixComposite(
		"${{ inputs.cmd }}",
		0,
		`echo "Running command: ${{ inputs.cmd }}"`,
	)
...
func TestADES106FixComposite(t *testing.T) {
	yamlfile := filepath.Join("vuln", "ADES106Composite.yaml")
	originalYAML, err := os.ReadFile(yamlfile)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	// Create the fix (this does NOT apply it yet)
	fix := autofix.ADES106FixComposite(
		"${{ inputs.value }}",
		0,
		"1 + parseInt(${{ inputs.value }})",
	)
...
// Need to fix remove ”
func TestADES107FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES107Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}
	...
	// Create the fix
	fix := autofix.ADES107FixComposite(
		"${{ inputs.color }}",
		0,
		"{ attachments: [{ color: '${{ inputs.color }}' }] }",
	)
...
// Change from ” to "" for interpolation
func TestADES108FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES108Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}

	fix := autofix.ADES108FixComposite(
		"${{ inputs.name }}",
		0,
		"echo 'Hello ${{ inputs.name }}'",
	)
...
// Need to fix for python interpolation because of double quotation
func TestADES109FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES109Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}

	fix := autofix.ADES109FixComposite(
		"${{ inputs.name }}",
		0,
		"print(\"Hello ${{ inputs.name }}\")",
	)
...
func TestADES110FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES110Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}

	fix := autofix.ADES110FixComposite(
		"${{ inputs.name }}",
		0,
		"Write-Output 'Hello ${{ inputs.name }}'",
	)
...
func TestADES111FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES111Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}

	fix := autofix.ADES111FixComposite(
		"${{ inputs.query }}",
		0,
		"yq '${{ inputs.query }}' 'config.yml'",
	)
...
func TestADES112FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES112Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}

	fix := autofix.ADES112FixComposite(
		"${{ inputs.query }}",
		0,
		"yq eval '${{ inputs.query }}' 'config.yml'",
	)
...
func TestADES113FixComposite(t *testing.T) {
	yamlpatch := filepath.Join("vuln", "ADES113Composite.yaml")
	originalYAML, err := os.ReadFile(yamlpatch)
	if err != nil {
		t.Fatalf("error reading YAML file: %v", err)
	}

	fix := autofix.ADES113FixComposite(
		"${{ inputs.name }}",
		0,
		"Write-Output 'Hello ${{ inputs.name }}'",
	)
